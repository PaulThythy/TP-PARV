...
void createParticules(int nbParticules)
{
  ...
  p.position[0] = 0.0f;
  p.position[1] = 0.0f;
  p.position[2] = 1.0f; // position initiale (0,0,1)

  p.vitesse[0] = 0.0f;
  p.vitesse[1] = 1.0f;
  p.vitesse[2] = 1.0f; // vitesse initiale (0,1,1)
...
void anim(int NumTimer)
{
  ...
  for(size_t i = 0;i < listeParticules.size();i++) {
    ...
    // Rebond sur le sol : si la particule pénètre en dessous de z = 0
    if (pos.z < 0.0f) {
      pos.z = 0.0f;                    // On remet la particule au niveau du sol
      velocity.z = -velocity.z * restitution; // Inversion de la composante verticale avec perte d'énergie
    }
